...
## Big picture
- Monorepo with platform services plus standalone tools. Platform = Next.js frontend + Express API + Postgres; tools = CLI + MCP servers. See [SERVICES.md](../docs/SERVICES.md).
...
- API serves registry/search data to frontend; CLI consumes API endpoints for install/search flows. See [SERVICES.md](../docs/SERVICES.md).
- Platform ports (local): frontend :3000, API :3002, Postgres :5434. See [SERVICES.md](../docs/SERVICES.md).
- CLI and MCP servers are **not** platform services; they are separate globally installed tools. See [SERVICES.md](../docs/SERVICES.md).
...
- API expects a PostgreSQL connection configured via `packages/api/.env` (Supabase commonly used). See [SERVICES.md](../docs/SERVICES.md).

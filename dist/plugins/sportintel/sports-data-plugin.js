...
exports.SportsDataPlugin = void 0;
const events_1 = require("events");
class SportsDataPlugin extends events_1.EventEmitter {
    providers = new Map();
    ...
    async selectOptimalProvider(request) {
        const eligibleProviders = Array.from(this.providers.values())
            ...
            if (!provider.dataTypes.includes(request.dataType))
                return false;
            // Check if provider is excluded
            if (request.excludeProviders?.includes(provider.id))
                return false;
            // Check if provider is preferred
            if (request.preferredProviders && !request.preferredProviders.includes(provider.id))
                return false;
    ...
    calculateRequestCost(provider, request) {
        const endpoint = provider.endpoints[request.dataType];
    ...
    async tryFallbackProviders(request, excludeProviderId) {
        ...
        try {
            ...
            this.logger.debug('Fallback provider also failed', { error: error.message });
            return null;
        }
    }
    setupEventListeners() {
    ...
    async handleProviderHealthCheck(event) {
...
exports.SportsDataPlugin = SportsDataPlugin;
//# sourceMappingURL=sports-data-plugin.js.map
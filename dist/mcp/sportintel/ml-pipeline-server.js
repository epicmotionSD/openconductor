...
exports.MLPipelineMCPServer = void 0;
exports.createMLPipelineMCPServer = createMLPipelineMCPServer;
const index_js_1 = require("@modelcontextprotocol/sdk/server/index.js");
const stdio_js_1 = require("@modelcontextprotocol/sdk/server/stdio.js");
const types_js_1 = require("@modelcontextprotocol/sdk/types.js");
class MLPipelineMCPServer {
    server;
    eventBus;
    sportsOracle;
    logger;
    ...
    constructor(config, eventBus, sportsOracle, logger) {
        this.config = config;
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
            ...
            if (this.activeInferences >= this.config.inference.maxConcurrentInference) {
                throw new types_js_1.McpError(types_js_1.ErrorCode.ResourceUnavailable, 'Maximum concurrent inferences exceeded. Please try again later.');
            }
    ...
    async handlePredictPlayerPerformance(args) {
        this.logger.debug('Handling predict_player_performance', { playerId: args.playerId });
        try {
            // Use SportsOracleAgent for prediction
            const prediction = await this.sportsOracle.predictPlayerPerformance(args.playerId, args.gameContext, {
            ...
            if (args.includeExplanation && this.config.explainability.enabled) {
                explanation = await this.generateExplanation('player_performance', prediction, args.gameContext);
            }
            const result = {
                success: true,
                data: {
            ...
            throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, `Failed to predict player performance: ${error.message}`);
        }
    }
    async handlePredictGameOutcome(args) {
        ...
        try {
            ...
            const prediction = await this.sportsOracle.predictGameOutcome(args.homeTeam, args.awayTeam, {
            ...
            let explanation = null;
            if (args.includeExplanation && this.config.explainability.enabled) {
                explanation = await this.generateExplanation('game_outcome', prediction, { homeTeam: args.homeTeam, awayTeam: args.awayTeam });
            }
            const result = {
                success: true,
                data: {
            ...
            throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, `Failed to predict game outcome: ${error.message}`);
        }
    }
    async handleOptimizeLineup(args) {
        this.logger.debug('Handling optimize_lineup', { contest: args.contest });
        try {
            // Use SportsOracleAgent for lineup optimization
            const optimization = await this.sportsOracle.optimizeLineups(args.playerPool, args.constraints, {
            ...
            throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, `Failed to optimize lineup: ${error.message}`);
        }
    }
    async handleExplainPrediction(args) {
        ...
        try {
            ...
            if (!explanation) {
                ...
                explanation = await this.generateDetailedExplanation(args.modelId, args.inputFeatures, {
            ...
            throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, `Failed to explain prediction: ${error.message}`);
        }
    }
    // Additional handler methods...
    async handleAnalyzeModelPerformance(args) {
        const model = this.modelRegistry.get(args.modelId);
        if (!model) {
            throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidRequest, `Model ${args.modelId} not found`);
        }
        const analysis = {
            modelId: args.modelId,
            performance: {
    ...
    // Helper methods
    async generateExplanation(type, prediction, context) {
    ...
    async generateDetailedExplanation(modelId, features, options) {
        return {
            modelId,
...
exports.MLPipelineMCPServer = MLPipelineMCPServer;
// Export factory function
function createMLPipelineMCPServer(config, eventBus, sportsOracle, logger) {
    return new MLPipelineMCPServer(config, eventBus, sportsOracle, logger);
}
//# sourceMappingURL=ml-pipeline-server.js.map
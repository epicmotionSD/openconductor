...
exports.createRegistryRoutes = createRegistryRoutes;
const express_1 = require("express");
const error_handler_1 = require("../middleware/error-handler");
/**
 * Create registry management routes
 */
function createRegistryRoutes(conductor, logger, errorManager) {
    const router = (0, express_1.Router)();
    /**
     * GET /registry/agents - Search agents in registry
     */
    router.get('/agents', (0, error_handler_1.asyncHandler)(async (req, res) => {
        const { q, type, tags, category, minRating, maxPrice, page = 1, limit = 20, sort = 'popularity', order = 'desc' } = req.query;
        ...
        try {
            ...
            if (searchQuery.tags) {
                filteredAgents = filteredAgents.filter(agent => searchQuery.tags.some((tag) => agent.tags.includes(tag)));
            }
            ...
            if (searchQuery.maxPrice) {
                filteredAgents = filteredAgents.filter(agent => !agent.price || agent.price <= searchQuery.maxPrice);
            }
            // Apply sorting
     ...
     */
    router.get('/agents/:id', (0, error_handler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
        logger.debug('Getting agent from registry', { agentId: id, userId: req.user?.id });
        try {
            const agent = getMockRegistryAgent(id);
            if (!agent) {
                throw new error_handler_1.NotFoundError('Agent in registry', id);
            }
            const response = {
                success: true,
                data: agent,
     ...
     */
    router.post('/agents', (0, error_handler_1.asyncHandler)(async (req, res) => {
        ...
        try {
            ...
            if (!agentData.name || !agentData.type || !agentData.version) {
                throw new error_handler_1.ValidationError('Missing required fields: name, type, version');
            }
     ...
     */
    router.put('/agents/:id', (0, error_handler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
     ...
     */
    router.delete('/agents/:id', (0, error_handler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
     ...
     */
    router.post('/agents/:id/download', (0, error_handler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
     ...
     */
    router.get('/agents/:id/stats', (0, error_handler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
     ...
     */
    router.get('/tags', (0, error_handler_1.asyncHandler)(async (req, res) => {
 ...
 */
function getMockRegistryAgents() {
    return [
        {
 ...
 */
function getMockRegistryAgent(id) {
 ...
 */
function generateDownloadTrend() {
    const trend = [];
    for (let i = 29; i >= 0; i--) {
        const date = new Date();

...
exports.SportsSentinelAgent = void 0;
const sentinel_agent_1 = require("../sentinel-agent");
class SportsSentinelAgent extends sentinel_agent_1.SentinelAgent {
    dataProviders = new Map();
    predictionMetrics = new Map();
    sportsDataMetrics;
    costTracker = new Map();
     ...
     */
    async monitorSportsDataQuality() {
        ...
        try {
            ...
            this.log('error', 'Sports data quality monitoring failed', { error: error.message });
            throw error;
        }
    }
    /**
    ...
    initializeProviderHealth(providerId, name) {
        return {
            providerId,
    ...
    async generateQualityAlerts(results) {
        const alerts = [];
    ...
    createDataFreshnessAlert(dataType, minutes) {
        return {
    ...
    createDataQualityAlert(qualityType, score) {
        return {
    ...
    calculateOverallDataHealth(results) {
        const issues = [];
        // Check critical thresholds
        if (results.dataFreshness.injuryReports > 15)
            issues.push('critical');
        if (results.dataQuality.completeness < 0.9)
            issues.push('critical');
        if (results.dataQuality.accuracy < 0.95)
            issues.push('critical');
        // Check warning thresholds
        if (results.dataFreshness.playerStats > 30)
            issues.push('warning');
        if (results.dataQuality.completeness < 0.95)
            issues.push('warning');
    ...
    updateSportsMetrics(results) {
        ...
        this.recordMetric('data_freshness_overall', Object.values(results.dataFreshness).reduce((a, b) => a + b, 0) / Object.keys(results.dataFreshness).length, new Date());
    }
    convertToStandardMetrics(results) {
        return {
            'data_completeness': results.dataQuality.completeness,
            'data_accuracy': results.dataQuality.accuracy,
            'avg_response_time': Object.values(results.apiHealth).reduce((sum, provider) => sum + provider.responseTime, 0) / Object.keys(results.apiHealth).length,
            'prediction_accuracy': Object.values(results.predictionAccuracy).reduce((sum, acc) => sum + acc, 0) / Object.keys(results.predictionAccuracy).length
        };
    }
    async runSportsHealthChecks() {
        const healthChecks = [];
    ...
    generateOptimizationRecommendations(results) {
        ...
        if (Object.values(results.predictionAccuracy).some((acc) => acc < 0.65)) {
            recommendations.push('Review and retrain underperforming prediction models');
        }
        return recommendations;
    }
    async assessInjuryImpact(playerId, newStatus) {
    ...
    getInjurySeverityLevel(previousStatus, newStatus) {
        if (newStatus === 'out')
            return 'critical';
        if (newStatus === 'doubtful' || (previousStatus === 'healthy' && newStatus === 'questionable'))
            return 'error';
        if (newStatus === 'questionable')
            return 'warning';
        return 'info';
    }
    getInjuryRecommendedActions(playerId, newStatus) {
        const actions = [
    ...
    async createPredictionAccuracyAlert(modelId, accuracy, alertType) {
        const alert = {
    ...
    async checkProviderAlerts(providerId, health) {
        // Response time alert
    ...
    async createProviderAlert(providerId, alertType, value) {
    ...
    startDataFreshnessMonitoring() {
        setInterval(async () => {
            try {
                ...
                this.log('error', 'Data freshness monitoring error', { error: error.message });
            }
        }, 300000); // Every 5 minutes
    }
    startPredictionAccuracyMonitoring() {
        setInterval(async () => {
            try {
                await this.checkPredictionAccuracy();
            }
            catch (error) {
                this.log('error', 'Prediction accuracy monitoring error', { error: error.message });
            }
        }, 3600000); // Every hour
    }
    startApiHealthMonitoring() {
        setInterval(async () => {
            try {
                ...
                this.log('error', 'API health monitoring error', { error: error.message });
            }
        }, 60000); // Every minute
    }
    startCostTracking() {
...
exports.SportsSentinelAgent = SportsSentinelAgent;
//# sourceMappingURL=sports-sentinel-agent.js.map
...
exports.SportsOracleAgent = void 0;
const oracle_agent_1 = require("../oracle-agent");
class SportsOracleAgent extends oracle_agent_1.OracleAgent {
    sportsModels = new Map();
    weatherService; // Will be injected via DI
    injuryTracker; // Will be injected via DI
     ...
     */
    async predictPlayerPerformance(playerData, options) {
        ...
        try {
            ...
            const explanation = await this.generateExplanation(features, prediction.shapValues || {}, playerData);
     ...
     */
    async predictGameOutcome(homeTeam, awayTeam, gameContext) {
     ...
     */
    async optimizeLineup(players, constraints) {
        ...
        try {
            ...
            const playerPredictions = await Promise.all(players.map(player => this.predictPlayerPerformance(player, {
            ...
            const optimizedLineup = await optimizer.optimize(playerPredictions, constraints);
     ...
     */
    async predictOwnership(playerData) {
    ...
    async extractPlayerFeatures(playerData) {
        return {
    ...
    async extractGameFeatures(homeTeam, awayTeam, context) {
        return {
    ...
    async extractOwnershipFeatures(playerData) {
        return {
            salary: playerData.salary || 0,
            projected_points: playerData.projectedPoints || 0,
    ...
    calculateWeatherImpact(weather) {
        ...
        if (weather.precipitation && weather.precipitation > 0.5)
            impact -= 0.15;
        if (weather.temperature && (weather.temperature < 32 || weather.temperature > 90))
            impact -= 0.05;
        return impact;
    }
    encodeWeatherConditions(weather) {
    ...
    calculatePlayerVariance(playerData) {
    ...
    async generateExplanation(features, shapValues, playerData) {
    ...
    identifyRiskFactors(playerData, prediction) {
        const risks = [];
    ...
    calculateWinProbability(features) {
    ...
    generateGameFactors(features, shapValues) {
    ...
    createLineupOptimizer(strategy) {
        // Factory method to create strategy-specific optimizers
        return {
            optimize: async (predictions, constraints) => {
    ...
    generateLineupExplanation(lineup, constraints) {
        return {
    ...
    getFactorDescription(feature, impact, playerData) {
    ...
    generateReasoningText(factors, playerData) {
    ...
    async handleLineupOptimization(request) {
        this.log('info', 'Lineup optimization requested', { strategy: request.strategy });
        // Handle async lineup optimization requests
    }
}
exports.SportsOracleAgent = SportsOracleAgent;
//# sourceMappingURL=sports-oracle-agent.js.map
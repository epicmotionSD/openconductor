...
export function createPluginRoutes(
        ...
        try {
          ...
          plugins = plugins.filter(plugin => {
            return Object.entries(filterObj).every(([key, value]) => {
              if (key === 'status') return plugin.status === value;
              if (key === 'category') return plugin.category === value;
              if (key === 'active') return plugin.active === value;
              return true;
            });
          });
        } catch (error) {
  ...
  router.post('/', asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (error.message?.includes('already installed')) {
        throw new ConflictError('Plugin is already installed');
      }
      
      throw new PluginError(error.message, { originalError: error.message });
    }
  }));
  ...
  router.post('/:id/activate', asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (plugin.active) {
        throw new ConflictError('Plugin is already active');
      }
  ...
  router.post('/:id/deactivate', asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!plugin.active) {
        throw new ConflictError('Plugin is already inactive');
      }
   ...
   */
  router.get('/registry/search', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
function getPluginsByCategory(plugins: Plugin[]): Record<string, number> {
 ...
 */
function getPluginsByStatus(plugins: Plugin[]): Record<string, number> {
  const result: Record<string, number> = {};
  plugins.forEach(plugin => {
 ...
 */
function calculateAverageRating(plugins: Plugin[]): number {
 ...
 */
function calculateTotalMemoryUsage(plugins: Plugin[]): number {

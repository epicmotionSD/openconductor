...
import { OpenConductorEvent } from '../../types/events';
...
export class WebSocketHandler {
   ...
   */
  private async handleAgentCommand(client: WebSocketClient, message: WebSocketMessage): Promise<void> {
   ...
   */
  private async handleWorkflowCommand(client: WebSocketClient, message: WebSocketMessage): Promise<void> {
    if (!client.isAuthenticated) {
   ...
   */
  private sendError(client: WebSocketClient, message: string, requestId?: string, error?: any): void {
    this.sendMessage(client, {
      type: 'error',
      data: {
        message,
  ...
  private subscribeToEvents(): void {
    this.conductor.events.subscribeToAll((event: OpenConductorEvent) => {

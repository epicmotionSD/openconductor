...
import { 
  ...
  defaultExplainableAIConfig,
  PredictionInput,
  ExplanationResult
} from '../../ai/sportintel/explainable-ai-engine';
...
export class EnhancedSportsOracleAgent {
  ...
  // Prediction tracking
  private activePredictions = new Map<string, any>();
  private predictionCache = new Map<string, EnhancedPrediction>();
  ...
  constructor(
    ...
    this.explainableAI = createExplainableAIEngine(
      {
        ...defaultExplainableAIConfig,
        enabled: config.explainableAI.enabled,
        visualization: {
          ...defaultExplainableAIConfig.visualization,
    ...
    try {
      ...
      if (this.shouldGenerateExplanation(prediction, options)) {
        try {
          ...
          enhancedPrediction.explanation = await this.explainableAI.explainPrediction(
            predictionInput,
            prediction.value
          );
  ...
  async optimizeLineups(
    ...
    try {
      ...
      const playerPredictions = await this.batchPredictPlayers(
        playerPool, 
        { includeExplanations: options.explainStrategy }
      );

      // Run optimization algorithm
      const optimizedLineups = await this.runLineupOptimization(
        playerPredictions,
        constraints,
        options
      );
      ...
      const strategy = await this.generateLineupStrategy(
        optimizedLineups,
        playerPredictions,
        options
      );
      ...
      const alternatives = await this.generateLineupAlternatives(
        optimizedLineups[0],
        playerPredictions,
        constraints,
        options
      ...
      if (options.explainStrategy) {
        for (let i = 0; i < Math.min(3, result.lineups.length); i++) {
          result.lineups[i].explanation = await this.explainLineupConstruction(
            result.lineups[i],
            playerPredictions,
            options
          );
  ...
  async updatePlayerPrediction(
    ...
    try {
      ...
      const newPrediction = await this.predictPlayerPerformance(
        playerId,
        newContext,
        { forceExplanation: options.explainChange }
      );

      // Explain the change if requested
      if (options.explainChange && previousPrediction) {
        const changeExplanation = await this.explainPredictionChange(
          previousPrediction,
          newPrediction,
          newContext
        );
  ...
  private async preparePredictionInput(
    playerId: string,
    gameContext: any,
    options: any
    ...
    return {
      ...
      playerContext: {
        ...
        recentPerformance: recentPerformance.map((p: any) => p.fantasyPoints)
      },
      metadata: {
  ...
  private async runPredictionModel(input: PredictionInput, model: any): Promise<any> {
  ...
  private shouldGenerateExplanation(prediction: any, options: any): boolean {
    if (!this.config.explainableAI.enabled) return false;
    if (options.forceExplanation) return true;
    if (this.config.explainableAI.alwaysExplain) return true;
  ...
  private async batchPredictPlayers(
    playerPool: any[],
    options: { includeExplanations?: boolean } = {}
  ): Promise<Map<string, EnhancedPrediction>> {
    const predictions = new Map<string, EnhancedPrediction>();
    const batchSize = 10;
      ...
      const batchPromises = batch.map(async (player) => {
        try {
          const prediction = await this.predictPlayerPerformance(
            player.id,
            player.gameContext || {},
            { forceExplanation: options.includeExplanations }
          );
  ...
  private async runLineupOptimization(
    playerPredictions: Map<string, EnhancedPrediction>,
    constraints: any,
    options: any
  ): Promise<any[]> {
  ...
  private constructLineup(players: any[], constraints: any, variation: number): any {
    ...
    for (const position of positions) {
      const availablePlayers = players.filter(p => 
        p.context.playerContext.position === position ||
  ...
  private async generateLineupStrategy(
    lineups: any[],
  ...
  private async generateLineupAlternatives(
    baseLineup: any,
    predictions: Map<string, EnhancedPrediction>,
    constraints: any,
    options: any
  ...
  private async explainLineupConstruction(
    lineup: any,
    predictions: Map<string, EnhancedPrediction>,
    options: any
  ): Promise<ExplanationResult> {
    // Create a synthetic prediction input for lineup explanation
    const lineupFeatures: Record<string, number> = {
      total_salary: lineup.totalSalary,
      projected_score: lineup.projectedScore,
      risk_score: lineup.riskScore,
      avg_ownership: lineup.lineup.reduce((sum: number, p: any) => sum + p.ownership, 0) / lineup.lineup.length,
      stack_correlation: this.calculateStackCorrelation(lineup.lineup),
      value_plays: lineup.lineup.filter((p: any) => p.projection / p.salary > 0.002).length
    };

    const syntheticInput: PredictionInput = {
  ...
  private async extractFeatures(playerData: any, gameContext: any, requestedFeatures?: string[]): Promise<Record<string, number>> {
    const allFeatures = {
    ...
    if (requestedFeatures && requestedFeatures.length > 0) {
      ...
      for (const feature of requestedFeatures) {
        if (feature in allFeatures) {
          filtered[feature] = allFeatures[feature as keyof typeof allFeatures];
        }
      }
      return filtered;
    }
  ...
  private async getMarketContext(playerId: string): Promise<any> {
  ...
  private async explainPredictionChange(
    previousPrediction: EnhancedPrediction,
    newPrediction: EnhancedPrediction,
    newContext: any
  ): Promise<any> {
  ...
  private generateCacheKey(playerId: string, gameContext: any, options: any): string {
    const key = { playerId, gameContext, options };
    return Buffer.from(JSON.stringify(key)).toString('base64');
  }

  private isCacheValid(cached: EnhancedPrediction): boolean {
    const age = Date.now() - cached.metadata.timestamp.getTime();
    return age < 300000; // 5 minutes
  }

  private async getLatestPrediction(playerId: string): Promise<EnhancedPrediction | null> {
  ...
  private getPositionBaseValue(position: string): number {
  ...
  private identifyChangeFactors(prev: EnhancedPrediction, current: EnhancedPrediction, context: any): string[] {
    return ['Injury report update', 'Weather forecast change', 'Line movement'];
  }

  private getTopChangeReason(context: any): string {
    return 'updated injury status and weather conditions';
  }

  private updatePredictionMetrics(time: number, cacheHit: boolean): void {
...
export function createEnhancedSportsOracleAgent(
  config: EnhancedOracleConfig,
  logger: Logger,
  eventBus: EventBus,
  dataManager: SportsDataManager
): EnhancedSportsOracleAgent {
  return new EnhancedSportsOracleAgent(config, logger, eventBus, dataManager);
}

export const defaultEnhancedOracleConfig: EnhancedOracleConfig = {

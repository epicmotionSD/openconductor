...
export class AgentRuntime {
  ...
  async unregisterAgent(agentId: string): Promise<void> {
    ...
    if (!agent || !runtime) {
      throw this.errorManager.createAgentError(
        'AGENT_NOT_FOUND',
  ...
  async startAgent(agentId: string): Promise<void> {
    ...
    try {
      ...
      const agentError = this.errorManager.createAgentError(
        'AGENT_START_FAILED',
  ...
  getAgentStatus(agentId: string): AgentStatus | null {
    ...
    return runtime.status === 'registered' ? 'idle' : runtime.status as AgentStatus;
  }
  ...
  on(event: string, callback: (...args: any[]) => void): void {
    ...
    this.eventBus.subscribe(event as any, callback as any);
  }
  ...
  private async createRuntime(agent: Agent): Promise<RuntimeInstance> {
  ...
  private createEnvironment(agent: Agent): RuntimeEnvironment {
    return {
      runtime: 'nodejs',
  ...
  private createSecurityPolicy(_agent: Agent): SecurityPolicy {
    return {
      sandboxed: true,
  ...
  private async initializeAgent(agent: Agent, runtime: RuntimeInstance): Promise<void> {
    if (agent.initialize) {
      await this.executeInRuntime(runtime, async () => {
  ...
  private async executeInRuntime<T>(
  ...
  private async destroyRuntime(agentId: string): Promise<void> {

...
export class AlertsMCPServer {
  private server: Server;
  private eventBus: EventBus;
  private sportsSentinel: SportsSentinelAgent;
  private logger: Logger;
  private config: AlertsServerConfig;
  ...
  private async initializeAlerting(): void {
  ...
  private async handleCreateAlertRule(args: any): Promise<any> {
  ...
  private async handleMonitorInjuryReports(args: any): Promise<any> {
    ...
    try {
      ...
      if (args.realTimeUpdates !== false) {
        const subscription = await this.eventBus.subscribe(
          ['injury.reported', 'injury.updated'], 
          async (event) => {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to set up injury monitoring: ${(error as Error).message}`
      );
    }
  }

  private async handleGetActiveAlerts(args: any): Promise<any> {
    ...
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to get active alerts: ${(error as Error).message}`
      );
    }
  }
  ...
  private async processInjuryAlert(event: any, config: any): Promise<void> {
    ...
    // Apply filters
    if (config.players.length > 0 && !config.players.includes(playerId)) return;
    if (config.teams.length > 0 && !config.teams.includes(teamId)) return;
    if (config.positions.length > 0 && !config.positions.includes(position)) return;
    
    // Check severity threshold
    const severityLevels = { 'minor': 1, 'moderate': 2, 'major': 3 };
    const thresholdLevel = severityLevels[config.severityThreshold as keyof typeof severityLevels] || 1;
    const eventLevel = severityLevels[severity as keyof typeof severityLevels] || 1;
    ...
    if (config.includeImpactAnalysis) {
      alert.data.fantasyImpact = await this.sportsSentinel.analyzeInjuryImpact(
        playerId,
        event.data
      );
    }
  ...
  private async evaluateRuleConditions(event: any, rule: any): Promise<boolean> {
  ...
  private isRateLimited(ruleId: string, rule: any): boolean {
  ...
  private async generateAlert(event: any, rule: any): Promise<any> {
    return {
      id: this.generateAlertId(),
      ruleId: rule.id,
      ruleName: rule.name,
  ...
  private generateAlertMessage(event: any, rule: any): string {
  ...
  private async setupRuleMonitoring(rule: any): Promise<void> {
  ...
  private getEventTypesForRule(ruleType: string): string[] {
  ...
  private async processRuleEvent(event: any, rule: any): Promise<void> {
    const alert = await this.generateAlert(event, rule);
    this.activeAlerts.set(alert.id, alert);
    await this.deliverAlert(alert);
  }
  ...
  private async handleTestAlertRule(args: any): Promise<any> {
    return { content: [{ type: 'text', text: JSON.stringify({ success: true, testResults: 'passed' }, null, 2) }] };
  }

  private async handleConfigureAlertChannels(args: any): Promise<any> {
...
export function createAlertsMCPServer(
  config: AlertsServerConfig,
  eventBus: EventBus,
  sportsSentinel: SportsSentinelAgent,
  logger: Logger

...
import { 
  SportsDataMCPServer, 
  createSportsDataMCPServer, 
  SportsDataServerConfig 
} from './sports-data-server';
import { 
  MLPipelineMCPServer, 
  createMLPipelineMCPServer, 
  MLPipelineServerConfig 
} from './ml-pipeline-server';
import { 
  AnalyticsMCPServer, 
...
export interface MCPServerRegistryConfig {
  ...
  servers: {
    sportsData: SportsDataServerConfig & { enabled: boolean };
    mlPipeline: MLPipelineServerConfig & { enabled: boolean };
    analytics: AnalyticsServerConfig & { enabled: boolean };
    alerts: AlertsServerConfig & { enabled: boolean };
  };
}
...
export class MCPServerRegistry {
  ...
  private sportsDataServer?: SportsDataMCPServer;
  private mlPipelineServer?: MLPipelineMCPServer;
  private analyticsServer?: AnalyticsMCPServer;
  private alertsServer?: AlertsMCPServer;

  // Dependencies
  private sportsDataPlugin: SportsDataPlugin;
  private sportsDataManager: SportsDataManager;
  private sportsOracleAgent: SportsOracleAgent;
  private sportsSentinelAgent: SportsSentinelAgent;

  // Registry state
  private registeredTools: Map<string, Tool> = new Map();
  private serverStatus: Map<string, string> = new Map();
  ...
  private async createMCPServers(): Promise<void> {
    ...
    if (this.config.servers.sportsData.enabled) {
      ...
      this.sportsDataServer = createSportsDataMCPServer(
        this.config.servers.sportsData,
        this.sportsDataPlugin,
        this.sportsDataManager,
        this.eventBus,
        this.logger
      );
    ...
    if (this.config.servers.mlPipeline.enabled) {
      ...
      this.mlPipelineServer = createMLPipelineMCPServer(
        this.config.servers.mlPipeline,
        this.eventBus,
        this.sportsOracleAgent,
        this.logger
      );
    ...
    if (this.config.servers.analytics.enabled) {
      ...
      this.analyticsServer = createAnalyticsMCPServer(
        this.config.servers.analytics,
        this.eventBus,
        this.sportsDataManager,
        this.logger
      );
    ...
    if (this.config.servers.alerts.enabled) {
      ...
      this.alertsServer = createAlertsMCPServer(
        this.config.servers.alerts,
        this.eventBus,
        this.sportsSentinelAgent,
        this.logger
      );
   ...
   */
  private async registerToolsWithOpenConductor(): Promise<void> {
    this.logger.info('Registering MCP tools with OpenConductor ToolRegistry...');

    const toolRegistrations: Array<{ server: string; tools: any[]; category: ToolCategory }> = [
      {
        server: 'sportsData',
    ...
    for (const { server, tools, category } of toolRegistrations) {
      if (!this.isServerEnabled(server)) continue;
      ...
      for (const mcpTool of tools) {
        try {
          ...
          const tool: Tool = {
            ...
            execute: async (input: any, context?: Record<string, any>) => {
   ...
   */
  private async callMCPServerTool(server: any, toolName: string, input: any): Promise<any> {
   ...
   */
  private validateMCPToolInput(schema: any, input: any): { valid: boolean; errors?: string[] } {
  ...
  async startAllServers(): Promise<void> {
    ...
    if (this.sportsDataServer) {
      startPromises.push(
        this.sportsDataServer.start().then(() => {
    ...
    if (this.mlPipelineServer) {
      startPromises.push(
        this.mlPipelineServer.start().then(() => {
    ...
    if (this.analyticsServer) {
      startPromises.push(
        this.analyticsServer.start().then(() => {
  ...
  async stopAllServers(): Promise<void> {
    ...
    if (this.sportsDataServer) {
      stopPromises.push(
        this.sportsDataServer.stop().then(() => {
    ...
    if (this.mlPipelineServer) {
      stopPromises.push(
        this.mlPipelineServer.stop().then(() => {
    ...
    if (this.analyticsServer) {
      stopPromises.push(
        this.analyticsServer.stop().then(() => {
   ...
   */
  getHealthStatus(): any {
    const serverHealth: Record<string, any> = {};
  ...
  // Helper methods
  private getEnabledServers(): string[] {
    return Object.entries(this.config.servers)
      .filter(([_, config]) => config.enabled)
      .map(([name, _]) => name);
  }

  private getRunningServers(): string[] {
    return Array.from(this.serverStatus.entries())
      .filter(([_, status]) => status === 'running')
      .map(([name, _]) => name);
  }

  private isServerEnabled(serverName: string): boolean {
    return (this.config.servers as any)[serverName]?.enabled || false;
  }

  private getServerInstance(serverName: string): any {
    switch (serverName) {
  ...
  private getOverallHealth(): string {
    ...
    if (runningCount === enabledCount && enabledCount > 0) return 'healthy';
  ...
  private getSportsDataTools(): any[] {
    return [
  ...
  private getMLPipelineTools(): any[] {
    return [
...
export const defaultMCPServerConfig: MCPServerRegistryConfig = {
  enabled: true,
  autoStart: true,

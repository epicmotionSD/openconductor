...
export interface AgentMemory {
  ...
  encryption?: boolean; // For sovereign control requirements
  config?: Record<string, any>;
}
...
export interface Agent extends AgentLifecycleMethods {
  ...
  // Event system for orchestration
  events: EventEmitter;
...
export interface PredictionAgent extends Agent {
  type: 'prediction';
  predict<TInput = any, TPrediction = any>(
    input: TInput,
    options?: {
      confidence?: number;
      timeHorizon?: number;
...
export interface AdvisoryAgent extends Agent {
  type: 'advisory';
  advise<TContext = any, TRecommendation = any>(
    context: TContext,
    options?: {

 ...
 */
export class SportIntelSubscriptionMiddleware {
   ...
   */
  enforceUsageLimit(limitType: keyof SubscriptionLimits, usageType?: 'prediction' | 'api_call') {
    return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      if (!req.user) {
        return res.status(401).json({ error: 'Authentication required' });
      }
   ...
   */
  requireActiveSubscription(options: MiddlewareOptions = {}) {
    return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      if (!req.user) {
        return res.status(401).json({ error: 'Authentication required' });
      }
   ...
   */
  protect(options: MiddlewareOptions = {}) {
    const middlewares = [
      this.authenticate(),
    ...
    if (options.usageType) {
      ...
      middlewares.push(this.enforceUsageLimit(limitType as keyof SubscriptionLimits, options.usageType));
    }
  ...
  private getResetTime(limitType: keyof SubscriptionLimits, subscription: any): string {
    const now = new Date();
  ...
  private getRateLimitForSubscription(subscription: any): number {
    if (!subscription) return 60; // Default rate limit
...
export const createSubscriptionMiddleware = (
  ...
  return {
    ...
    protect: (options: MiddlewareOptions) => middleware.protect(options),
  };
};
